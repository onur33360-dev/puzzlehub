...
Every future game picks the archetype matching its content type rather than inventing a seventh. This section is deliberately forward-looking past Water Sort — it's what keeps the eventual ~100-game catalog visually coherent.
...
- **No sampled audio.** §18 is written around the existing synthesized `GameAudio` engine's real ceiling, not around what a sample library could achieve — this is an honest scoping choice, not an oversight.
